...
| PostgreSQL runtime CI | Implemented | SQLAlchemy/psycopg, full migrations, advisory locks and connection pooling | PostgreSQL 17 integration plus concurrent audit/idempotency test | HA topology and PITR remain deployment infrastructure |
...
| OpenTelemetry and SLO alerts | Implemented with limitation | FastAPI/SQLAlchemy spans, OTLP/HTTP exporter, trace-correlated JSON logs and Prometheus alert rules | `test_telemetry.py`, observability tests and `promtool check rules` in CI | Collector/backend and alert routing require deployment configuration |
